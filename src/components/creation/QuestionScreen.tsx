...
const QuestionScreen: React.FC<QuestionScreenProps> = ({
  ...
  const regularOptions = stepData.options.filter(option => !stepData.customOption || option.title !== stepData.customOption.title);
  ...
  return <>
            ...
            <div className="touch-grid mb-6">
                {regularOptions.map(option => <div key={option.title} onClick={() => handleOptionSelect(option.title)} className={`
                            ...
                            ${isOptionSelected(option.title) ? `bg-white/20 border-2 ${theme.optionSelectedBorder} scale-105 shadow-lg ${theme.optionSelectedShadow}` : `bg-white/10 border-2 border-white/20 ${theme.optionHover} hover:scale-102`}
                        `}>
                        {stepData.multiSelect && <div className="absolute top-3 right-3">
                                <Checkbox checked={isOptionSelected(option.title)} className="bg-white/20 border-white/40 pointer-events-none" />
                            </div>}
                        <h4 className="responsive-text text-white font-mono text-center pr-8 text-lg font-medium">{option.title}</h4>
                        <p className="responsive-text-sm text-white/80 mt-2 font-mono text-center pr-8 font-extralight text-base">{option.description}</p>
                    </div>)}
            </div>
        ...
        return currentAnswers.length === 0;
            ...
            {hasCustomOption && <div className="w-full">
                    <div onClick={() => handleOptionSelect(stepData.customOption!.title)} className={`
                            ...
                            ${isOptionSelected(stepData.customOption!.title) ? `bg-white/20 border-2 ${theme.optionSelectedBorder} scale-105 shadow-lg ${theme.optionSelectedShadow}` : `bg-white/10 border-2 border-white/20 ${theme.optionHover} hover:scale-102`}
                        `}>
                        {stepData.multiSelect && <div className="absolute top-3 right-3">
                                <Checkbox checked={isOptionSelected(stepData.customOption!.title)} className="bg-white/20 border-white/40 pointer-events-none" />
                            </div>}
                    ...
                    {isOptionSelected(stepData.customOption!.title) && <div className="mt-6">
                            ...
                            <textarea id="custom-answer" rows={4} className={`w-full bg-white/10 border-white/20 border-2 rounded-lg touch-padding responsive-text text-white block transition ${theme.textAreaFocus} font-mono text-center touch-target ${profanityWarning ? 'border-red-500' : ''}`} value={customAnswers[stepData.id] || ''} onChange={handleTextChange} placeholder={stepData.customOption!.placeholder} onBlur={() => {
          ...
          if (customAnswers[stepData.id] && customAnswers[stepData.id].trim().length > 0) {
            setTimeout(() => {
              onAutoAdvance?.();
                            ...
                            {profanityWarning && <p className="text-red-400 responsive-text-sm mt-3 font-mono text-center">
